...
# ==============================================================================
#  PARTE 1 - MATEMATICAS
#  Todo lo de esta seccion es Python puro con numpy: se puede importar y probar
...
def homogenea(R: np.ndarray, p: Sequence[float]) -> np.ndarray:
    """Arma la matriz 4x4 a partir de una rotacion y una traslacion."""
    T = np.eye(4)
    T[:3, :3] = R
...
def inversa_homogenea(T: np.ndarray) -> np.ndarray:
    """Inversa de una transformacion homogenea (no se invierte la matriz completa)."""
    R, p = T[:3, :3], T[:3, 3]
    return homogenea(R.T, -R.T @ p)


def rpy_desde_R(R: np.ndarray) -> tuple[float, float, float]:
    """Angulos fijos ZYX: R = Rz(yaw) Ry(pitch) Rx(roll). Devuelve (roll, pitch, yaw)."""
    sy = math.hypot(R[0, 0], R[1, 0])
    if sy < 1e-9:  # pitch = +-90 grados, configuracion degenerada
...
def zyz_desde_R(R: np.ndarray) -> tuple[float, float, float]:
    ...
    return (math.atan2(R[1, 2], R[0, 2]), math.atan2(st, R[2, 2]), math.atan2(R[2, 1], -R[2, 0]))
...
def eje_angulo_desde_R(R: np.ndarray) -> tuple[np.ndarray, float]:
    """Todo giro equivale a rotar un angulo alrededor de un solo eje."""
    c = max(-1.0, min(1.0, (np.trace(R) - 1.0) / 2.0))
    ang = math.acos(c)
    if ang < 1e-9:
        return np.array([0.0, 0.0, 1.0]), 0.0
    if math.pi - ang < 1e-6:  # giro de 180 grados
